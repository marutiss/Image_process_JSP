...
<title>JSP 영상처리 (Beta 1) 서버</title>
</head>
<body>
...
inFp = new File("C:/images/raw/Etc_Raw(squre)/" + inFile);
...
switch(algo) {
	...
	case "301": //히스토그램 스트래칭
        ...
        for(int i=0;i<256;i++){ //룩업테이블 생성
        	int outV=(i-minValue)/(maxValue-minValue) * 255;
            if(outV<0)
                outV=0;
            if(outV>255)
                outV=255;
	...
	case "302": //엔드인
        ...
        for(int i=0;i<256;i++){ //룩업테이블 생성
        	int outV=(i-minValue1)/(maxValue1-minValue1) * 255;
            if(outV<0)
                outV=0;
            if(outV>255)
                outV=255;
...
outFp = new File("C:/images/raw/Etc_Raw(squre)/"+outFile);
...
out.println("<h1>" + outFile + " 저장 끝! </h1>");
%>
</body>
</html>